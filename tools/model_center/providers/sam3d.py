...
class Sam3DProvider(BaseProvider):
    spec = ProviderSpec(
        ...
        # The upstream setup guide asks for 32 GB.  The adapter's tested
        # native-spconv/sequential-release profile runs on a 23.6 GiB RTX 3090;
        # keep the official number as the recommendation, not a hard block.
        minimum_vram_gb=23.5,
    ...
    def generation_command(self, request: GenerationRequest, project_root: Path) -> list[str]:
        ...
        if options.get("sam3d_sequential_offload", options.get("sequential_offload", False)):
            command.append("--sam3d-sequential-offload")
        spconv_algo = options.get("sam3d_spconv_algo", options.get("spconv_algo"))
        if spconv_algo:
            command.extend(["--sam3d-spconv-algo", str(spconv_algo)])
