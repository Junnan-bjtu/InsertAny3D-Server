...
`tools/install_environments.sh` 准备环境。服务器机器配置和密钥只放在原 checkout 的 `.insertany3d/runtime.env`，不进入 Git。

## 入口

```bash
third_party/TRELLIS/.venv/bin/python tools/run_insert_pipeline.py --help
third_party/TRELLIS/.venv/bin/python tools/run_insert_batch.py --help
third_party/TRELLIS/.venv/bin/python tools/stage_adapter.py --help
```

低成本检查：

```bash
third_party/TRELLIS/.venv/bin/python -m py_compile tools/*.py tools/model_center/*.py
